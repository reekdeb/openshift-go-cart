...
Runs [Go](http://golang.org) on [OpenShift](https://openshift.redhat.com/app/login) using downloadable cartridge support.  To install to OpenShift from the CLI (you'll need version 1.9 or later of rhc), run:
    ...
    rhc create-app mygo https://cartreflect-claytondev.rhcloud.com/reflect?github=reekdeb/openshift-go-cart
...
The repository contains a sample go file which will print "hello, world" when someone hits your web application - see [web.go](https://github.com/reekdeb/openshift-go-cart/blob/master/template/web.go).
